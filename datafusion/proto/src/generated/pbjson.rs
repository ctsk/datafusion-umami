...
impl serde::Serialize for CompactExecNode {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("datafusion.CompactExecNode", len)?;
        if let Some(v) = self.input.as_ref() {
            struct_ser.serialize_field("input", v)?;
        }
        if self.threshold != 0. {
...
impl<'de> serde::Deserialize<'de> for CompactExecNode {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = CompactExecNode;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct datafusion.CompactExecNode")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<CompactExecNode, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                let mut input__ = None;
        ...
        deserializer.deserialize_struct("datafusion.CompactExecNode", FIELDS, GeneratedVisitor)
    }
}
        ...
        S: serde::Serializer,
    {
        ...
        if let Some(v) = self.physical_plan_type.as_ref() {
            match v {
                ...
                physical_plan_node::PhysicalPlanType::Compact(v) => {
                    struct_ser.serialize_field("compact", v)?;
                }
            }
        }
        struct_ser.end()
...
;
                        ...
                        GeneratedField::Compact => {
                            ...
                            physical_plan_type__ = map_.next_value::<::std::option::Option<_>>()?.map(physical_plan_node::PhysicalPlanType::Compact)
;
                        }
                    }
                }
